...
export const triggerRun = async (suiteId: number, caseId?: number, browser: string = "chromium", device?: string): Promise<TestRun | TestRun[]> => {
    let url = `/runs?suite_id=${suiteId}&browser=${browser}`;
    if (caseId) {
        url += `&case_id=${caseId}`;
    }
    if (device) {
