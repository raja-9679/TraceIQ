...
- **MOB-5. Port the AI differentiators** — ✅ shipped 2026-07-26: selector heal on Appium XML page source (`healMobileLocator` + runtime self-heal on unique match, same `SelectorHealProposal` pipeline as web) and mobile-aware failure-analysis heuristics. Remaining in this stage: mobile visual diff (pixelmatch reuse). The Chrome recorder does not carry over (native recording is out of scope).
